...
{
    public class SetTimeHandler : IRequestHandler<SetTimeRequest, Response>
    {
        private readonly IBombardierService _bombardierService;

        public SetTimeHandler(IBombardierService bombardierService)
        {
            _bombardierService = bombardierService;
        }

        public async Task<Response> Handle(SetTimeRequest request, CancellationToken cancellationToken)
        {
            try
            {
                await _bombardierService.SetTime(request.OrderId, request.SlotNumber, cancellationToken);
